...
def index(request):
    ...
    if talktome:
        log_message()
        return HttpResponse({'message': 'Email status logged'})

    return HttpResponse({'message': 'Enter ?sendmail=fajubemayowa@gmail.com OR ?talktome'})
...
def get_logs(request):
    try:
        ...
        return HttpResponse({'message': f'Failed to retrieve logs: {str(e)}'}, status=500)
