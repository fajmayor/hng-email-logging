...
def index(request):
    ...
    if talktome:
        log_message()
        return JsonResponse({'message': 'Email status logged'})

    return JsonResponse({'message': 'Enter ?sendmail=fajubemayowa@gmail.com OR ?talktome'})
...
def get_logs(request):
    try:
        ...
        return JsonResponse({'message': f'Retrieving logs Failed: {str(e)}'}, status=500)
